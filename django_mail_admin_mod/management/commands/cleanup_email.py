...
class Command(BaseCommand):
    ...
    def handle(self, verbosity, days, **options):
        ...
        count_outgoing = OutgoingEmail.objects.filter(
            created__lt=cutoff_date
        ).count()
        OutgoingEmail.objects.only('id').filter(
            created__lt=cutoff_date
        ).delete()
        print(
            "Deleted {0} outgoing mails created before {1} " . format(
                count_outgoing, cutoff_date
            )
        )
