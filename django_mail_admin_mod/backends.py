...
class CustomEmailBackend(EmailBackend):
    def __init__(self, host=None, port=None, username=None, password=None,
        ...
        self.username = (
            configuration.email_host_user if username is None else username
        )
        self.password = (
            configuration.email_host_password if password is None else password
        )
        self.use_tls = (
            configuration.email_use_tls if use_tls is None else use_tls
        )
        self.use_ssl = (
            configuration.email_use_ssl if use_ssl is None else use_ssl
        )
        self.timeout = (
            configuration.email_timeout if timeout is None else timeout
        )
        self.ssl_keyfile = (
            configuration.email_ssl_keyfile if ssl_keyfile is None
            else ssl_keyfile
        )
        self.ssl_certfile = (
            configuration.email_ssl_certfile if ssl_certfile is None
            else ssl_certfile
        )
