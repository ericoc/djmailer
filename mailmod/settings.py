...
def get_available_backends():
    ...
    if backend:
        warnings.warn('Please use the new MAILMOD["BACKENDS"] settings',
                      DeprecationWarning)
    ...
    backends['default'] = getattr(
        ...
        'mailmod.backends.CustomEmailBackend')

    # TODO: review this code later
    # # If EMAIL_BACKEND is set to use PostOfficeBackend
    # # and MAILMOD_BACKEND is not set, fall back to SMTP
    # if 'mailmod.EmailBackend' in backends['default']:
...
def get_cache_backend():
    if hasattr(settings, 'CACHES'):
        if "mailmod" in settings.CACHES:
            return get_cache("mailmod")
        else:
...
def get_config():
    ...
    """
    return getattr(settings, 'MAILMOD', {})
